...
export class RaccoltaSempliceService {
  ...
  public getRaccoltaSemplice(
    ...
    let url =
      ...
      CONTROLLERS_ENDPOINT.GET_DATI_RACCOLTA_SEMPLICE +
      "?codiceAzienda=" +
      aziendaCodice +
      "&from=" +
      dataInizio +
  ...
  public getFascicoliArgo(azienda: string, idUtente: string, value: string): Observable<HttpResponse<FascicoloArgo[]>> {
    let url =
      getInternautaUrl(BaseUrlType.Scrivania) +
      CONTROLLERS_ENDPOINT.GET_FASCICOLI_ARGO +
      "?azienda=" +
      azienda +
      "&idusr=" +
      idUtente +
  ...
  public getDocumentiArgo(
    azienda: string,
    idUtente: string,
    registro: string,
    value: string
  ): Observable<HttpResponse<DocumentoArgo[]>> {
    let url =
      getInternautaUrl(BaseUrlType.Scrivania) +
      CONTROLLERS_ENDPOINT.GET_DOCUMENTI_ARGO +
      "?azienda=" +
      azienda +
      "&idusr=" +
      idUtente +
  ...
  public downloadAllegato(azienda: String, idSottodocumento: String): Observable<any> {
    let url =
      getInternautaUrl(BaseUrlType.Scrivania) + CONTROLLERS_ENDPOINT.DOWNLOAD + "?azienda=" + azienda + "&id=" + idSottodocumento;
  ...
  public editRubrica(azienda: string, persona: PersonaRS, map: Map<string, string>): Observable<any> {
    ...
    if (this.notEmptyText(map.get("telefono"))) {
      ...
      if (this.notEmptyText(map.get("idTelefono"))) json = json + ' "idTelefono": "' + map.get("idTelefono") + '",';
    }

    if (this.notEmptyText(map.get("mail"))) {
      json = json + ' "mail": "' + map.get("mail") + '" ,';
      if (this.notEmptyText(map.get("idMail"))) json = json + ' "idMail": "' + map.get("idMail") + '",';
    }

    if (
      this.notEmptyText(map.get("via")) ||
      this.notEmptyText(map.get("civico")) ||
      this.notEmptyText(map.get("cap")) ||
      this.notEmptyText(map.get("comune")) ||
      this.notEmptyText(map.get("provincia")) ||
      this.notEmptyText(map.get("nazione"))
    ) {
      ...
      if (this.notEmptyText(map.get("idIndirizzo"))) json = json + ' "idIndirizzo": "' + map.get("idIndirizzo") + '",';
    }
