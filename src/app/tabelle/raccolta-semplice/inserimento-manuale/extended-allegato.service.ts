...
import { BaseUrlType, getInternautaUrl, AllegatoService } from "@bds/internauta-model";
...
export class ExtendedAllegatoService extends AllegatoService {
  ...
  public uploadAllegato(formData: FormData) {
    ...
    console.log(apiUrl);
    return this._http
      .post(apiUrl, formData, { reportProgress: true, observe: "events" })
  ...
  public downloadAttachment(dettaglioAllegato: DettaglioAllegato): Observable<any> {
    const url =
      getInternautaUrl(BaseUrlType.Scripta) + "/" + CUSTOM_SERVER_METHODS.downloadAttachment + "/" + dettaglioAllegato.id;
